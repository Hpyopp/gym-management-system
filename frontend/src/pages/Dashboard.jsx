...
const MemberDashboard = () => {
  ...
  const [error, setError] = useState(''); // Only used for the plan section now
  ...
  useEffect(() => {
    const fetchMyPlan = async () => {
      ...
      try {
        ...
        // 🔥 THE FIX: Don't crash the app! Just save the error message for the plan section.
        setError(err.response?.data?.message || 'Failed to load your protocol. Ask Admin to assign a plan.');
        setData(null); // Keep data null, but let the UI render!
      } finally {
        setLoading(false);
      }
    };
  ...
  // 🔥 NO MORE FULL-SCREEN RED ERROR BLOCK HERE! 🔥

  // Safe Extraction using Optional Chaining (Crash-Proof)
  const profile = data?.profile;
  const plan = data?.assignedPlan;
  const weight = data?.userWeight || '--';
  const isExpired = profile?.expiryDate ? new Date(profile.expiryDate) < new Date() : false;
  const daysLeft = calculateDaysLeft(profile?.expiryDate);
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '40px', paddingBottom: '20px', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
          <div>
            ...
            <p style={{ color: '#94a3b8', margin: 0, fontSize: '1rem', fontWeight: '500' }}>
              Current Mass: <span style={{ color: '#10b981', fontWeight: 'bold' }}>{weight} Kg</span>
            </p>
          </div>
        ...
        <div style={{ backgroundColor: '#1e293b', borderRadius: '16px', padding: '35px', marginBottom: '40px', border: '1px solid rgba(255,255,255,0.05)', display: 'flex', flexWrap: 'wrap', gap: '30px', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 20px 40px rgba(0,0,0,0.4)' }}>
          ...
          <div style={{ flex: 1, minWidth: '250px' }}>
            ...
            {profile ? (
              // IF BACKEND SENDS PROFILE, SHOW EXPIRY
              <>
                ...
                <div style={{ display: 'inline-block', backgroundColor: '#0f172a', padding: '10px 20px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.05)' }}>
                  <p style={{ color: '#64748b', fontSize: '0.85rem', margin: 0 }}>Valid till: <strong style={{color: '#f8fafc', fontSize: '1rem'}}>{new Date(profile.expiryDate).toLocaleDateString('en-IN')}</strong></p>
                </div>
              </>
            ) : (
              // IF BACKEND BLOCKS DATA, SHOW SAFE FALLBACK
              <div style={{ padding: '15px', backgroundColor: 'rgba(251, 191, 36, 0.1)', border: '1px dashed #fbbf24', borderRadius: '8px' }}>
                <p style={{ color: '#fbbf24', margin: 0, fontWeight: 'bold', fontSize: '0.95rem' }}>Profile details temporarily unavailable.</p>
                <p style={{ color: '#94a3b8', margin: '5px 0 0 0', fontSize: '0.85rem' }}>Your QR Code is still fully active for front desk entry.</p>
              </div>
            )}
          </div>
          
          {/* THE FIXED, STRAIGHT, PREMIUM QR CODE (ALWAYS RENDERS) */}
        ...
        {plan ? (
          ...
          <div style={{ textAlign: 'center', padding: '60px 20px', backgroundColor: '#1e293b', borderRadius: '16px', color: '#94a3b8', border: '1px dashed rgba(239, 68, 68, 0.5)' }}>
            <h2 style={{ fontSize: '1.8rem', color: '#f8fafc', marginBottom: '15px' }}>⚠️ {error || 'No Protocol Assigned'}</h2>
            <p style={{ fontSize: '1.05rem', maxWidth: '500px', margin: '0 auto', lineHeight: '1.6' }}>
              Your profile is active, but your Gym Owner hasn't assigned a routine for your weight bracket yet. Your QR code is fully functional. Please contact the front desk for your diet plan.
            </p>
          </div>
        )}
